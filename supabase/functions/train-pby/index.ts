...
function json(body: unknown, status = 200, clientMaxAge = 600, edgeMaxAge = 600) { // 10m CDN cache
...
Deno.serve(async (req) => {
  ...
  if (!appId || !appKey) return json({ error: "Missing TransportAPI credentials" }, 500);
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (row) return json(row.payload);
    ...
    const mapped = list.slice(0, 10).map((d: any) => ({
